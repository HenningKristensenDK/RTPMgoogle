...
import { CORRESPONDENCE_STATUS_LABEL as STEP_LABEL, formatDateYMD, pickResponsible } from "../../lib/format";
...
export default function CorrespondenceStatusBar({ item, roles, onChangeStatus }: Props) {
  const [pending, setPending] = useState<CorrespondenceStatus | null>(null);
  const isObsolete = item.status === "obsolete";
  const currentIdx = isObsolete ? -1 : CORRESPONDENCE_TRACK_STATUSES.indexOf(item.status);
  ...
  function lastEntry(status: CorrespondenceStatus) {
    return [...history].reverse().find((h) => h.to === status);
  }

  function ownerLabel(status: CorrespondenceStatus): string {
  ...
  return (
    <div className="mb-3 rounded-card bg-white px-6 py-4 shadow-card">
      ...
      <div className={`flex items-start ${isObsolete ? "opacity-40" : ""}`}>
        {CORRESPONDENCE_TRACK_STATUSES.map((status, idx) => {
          const isGreen = !isObsolete && idx <= currentIdx;
          const isNext = !isObsolete && idx === currentIdx + 1;
          const isLast = idx === CORRESPONDENCE_TRACK_STATUSES.length - 1;
